...
class KLKalmanFilter(object):
    ...
    **Example**
    ...
    def update(self, z, theta=None, c=None, R=None, H=None):
        """
        ...
        # get theta
        def f(t):
            M = self.inv(self._I - t*self.P)
            (sign, neglogdet) = np.linalg.slogdet(M)
            tr = np.trace(M - self._I)
            return .5*(tr - neglogdet) - self.c
        ...
        if theta:
            ...
            if c:
                self.c = c
            r = eigh(self.P, eigvals=(self.dim_x-1,self.dim_x-1), eigvals_only=True)[0] 
            sol = root_scalar(f, bracket=[0+1e-8,1/r-1e-8])
            self.theta = sol.root
            
        # update P
        self.P = self.inv(self.inv(self.P) - self.theta * self._I)
